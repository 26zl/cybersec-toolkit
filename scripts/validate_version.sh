...
try:
    ...
    # OCI packages must not carry a 'version' field (registry rule); the version
    # lives in the image tag inside 'identifier'.
    server_top = server["version"]
    server_tag = server["packages"][0]["identifier"].rsplit(":", 1)[1]
    vals = [plugin_version, marketplace_version, server_top, server_tag]
...
print(plugin_version, marketplace_version, server_top, server_tag, sep="\t")
PY
)"; then
...
IFS=$'\t' read -r v_plugin v_marketplace v_server v_server_tag <<< "$json_versions"
